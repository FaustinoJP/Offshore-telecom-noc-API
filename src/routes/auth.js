...
router.post('/login', async (req, res) => {
  ...
  if (!email || !password) return res.status(400).json({ success: false, error: { code: 'BAD_REQUEST', message: 'Email and password required' } });
  const { rows } = await db.query('SELECT id, email, password, role FROM users WHERE email = $1', [email]);
  const user = rows[0];
